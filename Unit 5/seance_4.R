...
data_ressources = read.csv2("Unit5_Resources.csv", header = T)
Resource = data_ressources$Resource
Price = data_ressources$Price
...
data_resistance<-read.csv2("Unit5_Resistance.csv", header=T)
A <- data_resistance$A
B <- data_resistance$B
...
steel_tension = read.csv2("steel_tension.csv", header = T)
Material = steel_tension$Material
Process = steel_tension$Process
UTS = steel_tension$UTS
Material_Factor <- as.factor(Material)
Process_Factor = as.factor(Process)
aggregate(UTS~Material_Factor, FUN="mean")
#get the ANOVA table
anova_material <- aov(lm(UTS~Material_Factor))
summary(anova_material)

aggregate(UTS~Process_Factor, FUN="mean")
#get the ANOVA table
anova_Process <- aov(lm(UTS~Process_Factor))
summary(anova_Process)

anova_two_factors <- aov(lm(UTS ~ Material_Factor * Process_Factor))
print(summary(anova_two_factors))

interaction.plot(Material_Factor, Process_Factor,  UTS,
                 xlab="Material_Factor", 
                 ylab="UTS",
                 trace.label = "Process_Factor", 
                 main="Interaction Plot",
                 col = c("blue","red","green"), bg=c("blue","red","green"), 
                 pch=c(18,24,22), type="b"
)